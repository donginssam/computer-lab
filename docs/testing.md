...
전체 저장소 형식 검사는 `pnpm format:check`이며 `pnpm build`의 첫 단계에서도 실행합니다. `pnpm lint`는 oxlint의 정적 검사만 하고 포맷은 보지 않으므로, PR 워크플로는 둘을 따로 실행합니다. 타입 검사만 필요하면 `pnpm typecheck`를 실행합니다. 문서만 변경할 때는 문서 범위의 검사와 링크 확인으로 검증하고, 기존 파일의 포맷을 일괄 수정하지 않습니다.
